        ...
        while (flag) {
            ...
            if (command.toUpperCase().equals("Q")) {
                ...
                try {
                    ...
                    System.out.println("Please enter the correct value for menu selection.");
                }
            }
        }
    ...
    public static int readFile(File phile) throws java.io.FileNotFoundException {
        ...
        while (sc.hasNext()) {
            ...
            if (records == null) {
                ...
                if (checkCustomerExist(temp.getAccountID(), records)) {
                    System.out.println("Duplicate Account ID: " + temp.getAccountID());
                } else {
                     ...
                     */
                    if ((pos = getInsertPosition(temp, records)) >= 0) {
                        // insert somewhere in the middle
                        records = cloneCustomerArrayAndInsert(records, temp, pos + 1);
                    } else {
                        if (pos == -1) {
                            // insert in front of all records
                            records = cloneCustomerArrayAndInsert(records, temp, 0);
                        } else if (pos == -2) {
                            // insert behind all records
                            records = cloneCustomerArrayAndInsert(records, temp, records.length);
                        }
                    }
        ...
        while (sc.hasNext()) {
            ...
            if (keyvalue.length() > 0) {
                ...
                if (key.equals("ACCOUNT ID")) {
                    cust.setAccountID(Integer.parseInt(value));
                }
                ...
                if (key.equals("NAME")) {
                    try {
                        ...
                        cust.setName(value.substring(0, 20));
                ...
                if (key.equals("ADDRESS")) {
                    try {
                        ...
                        cust.setAddress(value.substring(0, 80));
                ...
                if (key.equals("DOB")) {
                    try {
                        ...
                        cust.setDOB(value.substring(0, 10));
                ...
                if (key.equals("PHONE NUMBER")) {
                    try {
                        ...
                        cust.setPhone(value.substring(0, 8));
                        System.out.println("Warning: Phone length greater than 8, truncating...");
                    }
                }

                if (key.equals("ACCOUNT BALANCE")) {
                    cust.setBalance(Double.parseDouble(value));
                }
            } else {
                // if keyvalue pair is empty, it means its a newline, signifying
                // end of account details, hence return customer class.
                return cust;
            }
        }
