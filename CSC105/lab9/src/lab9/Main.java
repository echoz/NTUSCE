...
public class Main {
            ...
            try {
                ...
                switch (sc.nextInt()) {
                    case 1:
                        ...
                        System.out.print("\nPhile for T1: ");
                        try {
                            System.out.println(Integer.toString(readFile(new File(getInput()))) + " records read.");
                        } catch (java.util.NoSuchElementException nseex) {
                            System.out.println("Wrong format");
                        } catch (FileNotFoundException fnfe) {
                            System.out.println("File does not exist. " + fnfe.toString());
                        } catch (Exception e) {
                    ...
                    case 6:
                        System.out.print("\nName1 for destination merge: ");
                        tempstring = getInput();
                        System.out.print("\nName2 for source merge: ");
                        try {
                            if (merge(tempstring, getInput())) {
                                System.out.println("Merge success");
                            } else {
                                System.out.println("Error: Merge failure! " +
                                        "Possible that name entered is wrong. " +
                                        "Make sure its exactly the same, ie. " +
                                        "case sensitive.");
                            }
                        } catch (Exception e) {
                            System.out.println(e.toString());
                    ...
                    case 7:
                        System.out.print("\nID to hunt for: ");
                        try {
                            grandUncle(t1, Integer.parseInt(getInput()));
                        } catch (Exception e) {
                            System.out.println(e.toString());
                        }
                    break;
    ...
    public static boolean merge(String name1, String name2) {
        ...
        node1 = search(t1, name1);
        node2 = search(t1, name2);

        if (node1 != null && node2 != null) {
            node1.getItem().setName(node1.getItem().getName() + " & " +
                    node2.getItem().getName());
            node1.getItem().setBalance(node1.getItem().getBalance() +
                    node2.getItem().getBalance());
            t1.remove(node2.getItem().getAccountID());
            return true;
        } else {
            return false;
        }
    ...
    private static BSTNode search(BST custs, String name) {
        return preOrderTTSearch(custs.getRoot(), name);
    }

    private static BSTNode preOrderTTSearch(BSTNode cur, String name) {
        BSTNode result1 = null, result2 = null;
        if (cur == null) return null;
        if (cur.getItem().getName().equals(name)) {
            return cur;
        }
        result1 = preOrderTTSearch(cur.getLeft(), name);
        result2 = preOrderTTSearch(cur.getRight(), name);
        if (result1 != null) {
            return result1;
    ...
    public static void grandUncle(BST cust, int id) {
        Queue grandfathers = new Queue();
        Queue fathers = new Queue();
        ...
        while (flag) {
            ...
            if (cur != null) {
                ...
                fathers = sons;
                sons = new Queue();
            } else if (count >= cust.getCount()) {
                flag = false;
            }
        }

        if (grandfathers.size() > 1) {
            grandfathers.rewind();

            System.out.println("Grand uncle nodes: ");
            while (grandfathers.hasNext()) {
                System.out.print(((Customer) grandfathers.next().getItem()).getAccountID() + " ");
            }
            System.out.println("\n");
        } else {
            System.out.println("No granduncle nodes");
        }
    ...
    public static int readFile(File phile) throws java.io.FileNotFoundException, java.text.ParseException {
    ...
    private static Customer parseCustomerDetails(Scanner sc) throws java.text.ParseException {
